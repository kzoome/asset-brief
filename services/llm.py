...
이 ETF가 추종하는 테마·섹터를 파악하고, 오늘자 뉴스 검색에 최적화된 짧고 구체적인 키워드를 만들어줘.
- ETF 자체 이름/티커가 아닌, ETF가 담고 있는 산업·테마의 뉴스를 잡을 수 있어야 해.
- 검색어는 2~4단어 이내로 짧게. "동향", "산업", "현황" 같은 일반 명사는 붙이지 마.
- 뉴스가 실제로 많이 생성되는 핵심 키워드 위주로.
